...
public interface IDAOLot extends JpaRepository<Lot,Integer>{
	...
	@Query("select l from Lot l left join fetch l.achats where l.acheteur=:acheteur")
	public List<Lot> findByAcheteurWithAchat(@Param("acheteur") User acheteur );
	
	@Query("select l from Lot l left join fetch l.achats where l.vendeur=:vendeur")
